  ...
  setup() {
    ...
    function getSlotObject(groupItem: GroupProperties) {
      return {
        ...
        default: () => <DynamicBinder {...{onFieldChange: onFieldChange}} fieldDefine={groupItem.properties} v-model={contextState.businessObject} />,
      };
    }
    ...
    return () => (
      <>
        {contextState.isActive && contextState.businessObject && contextState.activeBindDefine && (
          <>
            <div class="bpmn-panel-shrinkage" onClick={() => (panelState.shrinkageOff = !panelState.shrinkageOff)}>
              {panelState.shrinkageOff ? <i class="el-icon-s-fold" /> : <i class="el-icon-s-unfold" />}
            </div>
